...
def create_app(test_config=None):
  ...
  def delete_question(question_id):
      '''
      Delete question by quetions id
      Will abort is question id is not found
      '''
      try:
  ...
  def create_quiz():
      '''
      Creates quiz by quiz_category (0=all categories)
      Uses previous_questions to prevent duplicating questions in one quiz
      Aborts if category is invalid
      '''
      body = request.get_json()
      ...
      try:
          ...
          elif len(Question.query.filter(Question.category == category_id).all()) > 0:
              current_questions = Question.query.filter(Question.category == category_id).filter(Question.id.notin_(previous_questions)).all()

          if len(current_questions) > 0:
              next_question = current_questions[random.randrange(0, len(current_questions))]
              ...
              return jsonify ({
                'success': True,
                'question': 'No more question from this category or categories.'
              })
